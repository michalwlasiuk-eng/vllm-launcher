...
class MainWindow(QMainWindow):
    ...
    def _launch_llama(self, info: ModelInfo) -> None:
        ...
        # Jeśli w katalogu jest więcej niż jeden plik .gguf, poproś użytkownika o wybór
        if info.format == ModelFormat.GGUF:
            gguf_candidates = list(Path(info.path).glob("*.gguf"))
            if len(gguf_candidates) > 1:
                # otwórz dialog wyboru pliku GGUF
                selected, _ = QFileDialog.getOpenFileName(
                    self, "Wybierz plik GGUF", str(info.path), "Pliki GGUF (*.gguf)"
                )
                if selected:
                    model_arg = selected
